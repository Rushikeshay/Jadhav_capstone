class PhotosController < ApplicationController
  ...
  def update
    ...
    @photo.caption = params.fetch("query_caption", @photo.caption)
    @photo.image = params.fetch("query_image", @photo.image)

    if @photo.save
      redirect_to("/activities/#{@photo.activity_id}", { :notice => "Photo updated." })
    else
      render("photos/edit")
    end
  end
